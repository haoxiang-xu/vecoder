...
const DirItem = ({
  ...
  const [expandIconClassName, setExpandIconClassName] = useState(
    file.fileExpend
      ? "explorer_icon arrow_icon_down"
      : "explorer_icon arrow_icon_right"
  );
  ...
  useEffect(() => {
    if (expanded) {
      setExpandIconClassName("explorer_icon arrow_icon_down");
    } else {
      setExpandIconClassName("explorer_icon arrow_icon_right");
    }
  }, [expanded]);
    ...
    if (!expanded) {
      ...
      setTimeout(() => {
        setExpandIconClassName(
          "explorer_icon arrow_icon_down no_animation"
        );
      }, expandingTime * 1000);
      setExpanded(true);
      ...
      setTimeout(() => {
        setExpandIconClassName(
          "explorer_icon arrow_icon_right no_animation"
        );
      }, expandingTime * 1000);
      setExpanded(false);
  ...
  useEffect(() => {
    if (onSingleClickFile !== null) {
      if (onSingleClickFile.filePath === file.filePath) {
        setFileNameClassName("explorer_item selected");
      } else {
        setFileNameClassName("explorer_item");
      }
    }
  }, [onSingleClickFile]);
  ...
  const handleRenameInputOnKeyDown = async (event) => {
    if (event.key === "Enter") {
      ...
      if (!parentCheckNameExist(renameInput)) {
        ...
        setInputBoxClassName("explorer_input shake_component_animation");
        setTimeout(() => {
          setInputBoxClassName("explorer_input");
        }, 160);
      }
    }
    if (event.key === "Escape") {
            ...
            <div>
              {onCommand !== "false" ? (
                ...
                >
                  <img
                    src={arrow_icon}
                    className="explorer_icon unexpandable_icon"
                    loading="lazy"
                  />
                  {filename}
                </span>
            ...
            >
              {fileIcon !== undefined ? (
                <img
                  ...
                  className="explorer_icon script_icon"
                  loading="lazy"
                  draggable={"false"}
                ></img>
              ) : (
